    ...
    def __init__(
        ...
        **kwargs
        ...
        if access_token and refresh_token:
            token.update({"access_token": access_token, "refresh_token": refresh_token})
        if expires_at:
...
class Fitbit(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        for resource in Fitbit.RESOURCE_LIST:
            ...
            setattr(
                self, underscore_resource, curry(self._COLLECTION_RESOURCE, resource)
            )
        ...
        for qualifier in Fitbit.QUALIFIERS:
            setattr(
                self,
                "%s_activities" % qualifier,
                curry(self.activity_stats, qualifier=qualifier),
            )
            setattr(
                self,
                "%s_foods" % qualifier,
                curry(self._food_stats, qualifier=qualifier),
            )
        self.response = None
    ...
    def make_request(self, *args, **kwargs):
        ...
        if (
            ...
            raise exceptions.RateLimited(
                self.rate_limit_limit, self.rate_limit_remaining, self.rate_limit_reset
            )
    ...
    def activities_weekly_goal(self, distance=None, floors=None, steps=None):
        ...
        * ``steps`` -- New goal value; in an integer format
        """
        data = self._filter_nones(
            {"distance": distance, "floors": floors, "steps": steps}
        )
    ...
    def time_series(
        self, resource, user_id=None, base_date="today", period=None, end_date=None
    ):
        """
        ...
        if end_date:
            ...
            if not period in Fitbit.PERIODS:
                raise ValueError("Period must be one of %s" % ",".join(Fitbit.PERIODS))
            end = period
        ...
        * https://dev.fitbit.com/docs/activity/#get-frequent-activities
        ...
        if qualifier:
            if qualifier in Fitbit.QUALIFIERS:
                ...
                raise ValueError(
                    "Qualifier must be one of %s" % ", ".join(Fitbit.QUALIFIERS)
                )
        else:
            qualifier = ""

        url = "{0}/{1}/user/{2}/activities{qualifier}.json".format(
            *self._get_common_args(user_id), qualifier=qualifier
        )
        return self.make_request(url)
    ...
    def add_favorite_activity(self, activity_id):
        ...
        url = "{0}/{1}/user/-/activities/favorite/{activity_id}.json".format(
            *self._get_common_args(), activity_id=activity_id
        )
    ...
    def add_favorite_food(self, food_id):
        ...
        url = "{0}/{1}/user/-/foods/log/favorite/{food_id}.json".format(
            *self._get_common_args(), food_id=food_id
        )
        return self.make_request(url, method="POST")
    ...
    def get_alarms(self, device_id):
        ...
        url = "{0}/{1}/user/-/devices/tracker/{device_id}/alarms.json".format(
            *self._get_common_args(), device_id=device_id
        )
        return self.make_request(url)
    ...
    def update_alarm(
        ...
        for day in week_days:
            if day not in self.WEEK_DAYS:
                raise ValueError("Incorrect week day %s. see WEEK_DAY_LIST." % day)
        url = (
            "{0}/{1}/user/-/devices/tracker/{device_id}/alarms/{alarm_id}.json".format(
    ...
    def get_sleep(self, date):
        ...
        url = "{0}/{1}/user/-/sleep/date/{year}-{month}-{day}.json".format(
            *self._get_common_args(), year=date.year, month=date.month, day=date.day
        )
        return self.make_request(url)
    ...
    def search_foods(self, query):
        ...
        url = "{0}/{1}/foods/search.json?{encoded_query}".format(
            *self._get_common_args(), encoded_query=urlencode({"query": query})
        )
        return self.make_request(url)
    ...
    def _get_body(
        self, type_, base_date=None, user_id=None, period=None, end_date=None
    ):
        if not base_date:
    ...
    def get_friends_leaderboard(self, period):
        ...
        url = "{0}/{1}/user/-/friends/leaders/{period}.json".format(
            *self._get_common_args(), period=period
        )
        return self.make_request(url)
    ...
    def respond_to_invite(self, other_user_id, accept=True):
        ...
        url = "{0}/{1}/user/-/friends/invitations/{user_id}.json".format(
            *self._get_common_args(), user_id=other_user_id
        )
    ...
    def subscription(
        self, subscription_id, subscriber_id, collection=None, method="POST"
    ):
        """
